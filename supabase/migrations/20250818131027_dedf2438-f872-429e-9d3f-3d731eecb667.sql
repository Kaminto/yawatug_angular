...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE c.relname = 'transaction_fee_settings' 
...
BEGIN
  IF NOT EXISTS (
    ...
    AND tablename = 'transaction_fee_settings' 
    AND policyname = 'Admins can manage transaction fee settings'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'transaction_fee_settings' 
    AND policyname = 'Users can view transaction fee settings'
  ) THEN
...
INSERT INTO public.transaction_fee_settings (transaction_type, percentage_fee, flat_fee, currency, minimum_fee, maximum_fee) 
SELECT * FROM (VALUES
...
) AS v(transaction_type, percentage_fee, flat_fee, currency, minimum_fee, maximum_fee)
WHERE NOT EXISTS (
  SELECT 1 FROM public.transaction_fee_settings 
  WHERE transaction_fee_settings.transaction_type = v.transaction_type 
  AND transaction_fee_settings.currency = v.currency
);