-- Data Recovery Migration: Restore Lost Referral Earnings
-- This migration reconstructs missing referral commission data from transaction history

-- Step 1: Create a temporary table to track recovery operations
CREATE TABLE IF NOT EXISTS temp_referral_recovery_log (
...
-- Step 2: Create function to recover referral commissions from transaction history
...
BEGIN
    ...
    SELECT base_commission_rate INTO commission_rate
    ...
    LOOP
        ...
        INSERT INTO temp_referral_recovery_log (
            operation_type,
            user_id,
            transaction_id,
            commission_amount,
            ...
            transaction_record.transaction_id,
            commission_amount,
            'Recovered commission for referrer ' || transaction_record.referrer_id
        );
    ...
    -- Step 3: Update referral statistics with recovered data
    ...
    INSERT INTO temp_referral_recovery_log (
        ...
        'recovery_summary',
        'Recovery completed: ' || recovery_stats::text
    );
...
-- Step 4: Create function to validate referral data integrity
...
-- Step 5: Execute the recovery process
...
-- Step 7: Create permanent audit table for future tracking
...
-- Step 8: Create trigger to prevent future data loss during migrations
...
-- Step 9: Log the completion of data recovery
INSERT INTO referral_data_audit (
    audit_type,
    operation_details
) VALUES (
...
-- Step 10: Clean up temporary recovery log (move to permanent audit)
INSERT INTO referral_data_audit (
    audit_type,
    user_id,
    operation_details
...
FROM temp_referral_recovery_log;

-- Drop temporary table
DROP TABLE IF EXISTS temp_referral_recovery_log;