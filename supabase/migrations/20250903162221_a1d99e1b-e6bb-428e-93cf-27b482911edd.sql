...
INSERT INTO referral_earnings (referrer_id, referred_id, earning_amount, earning_rate, source_amount, source_type, currency, status)
SELECT DISTINCT
  p.referred_by as referrer_id,
  st.user_id as referred_id,
  st.total_amount * 0.05 as earning_amount,
  0.05 as earning_rate,
  st.total_amount as source_amount,
  'share_purchase' as source_type,
  st.currency,
  'pending' as status
...
WHERE st.status = 'completed' 
  ...
  AND NOT EXISTS (
    ...
    AND re.source_amount = st.total_amount
  );
