-- Fix process_booking_payment function to handle average_buy_price column
...
BEGIN
  ...
  IF booking_completed THEN
    ...
    DO UPDATE SET
      ...
      average_buy_price = ((user_share_holdings.average_buy_price * user_share_holdings.quantity) + (booking_record.booked_price_per_share * booking_record.quantity)) / (user_share_holdings.quantity + booking_record.quantity),
      total_invested = user_share_holdings.total_invested + booking_record.total_amount,
      updated_at = NOW();
  END IF;
