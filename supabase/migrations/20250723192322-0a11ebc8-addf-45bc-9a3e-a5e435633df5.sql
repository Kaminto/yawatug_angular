...
-- Task 1: Enable RLS on all tables without RLS policies
...
ALTER TABLE daily_fee_collections ENABLE ROW LEVEL SECURITY;
...
-- Create RLS policies for daily_fee_collections (admin view only)
CREATE POLICY "Admins can view fee collections" 
ON daily_fee_collections FOR SELECT 
TO authenticated 
USING (is_admin(auth.uid()));
