...
INSERT INTO sms_templates (template_name, template_category, purpose, message_template, variables, is_active, language) VALUES
('OTP Verification', 'security', 'otp_verification', 'YAWATU OTP: Your verification code is {{otp_code}}. Valid for {{expiry_minutes}} minutes. Do not share this code.', '["otp_code", "expiry_minutes"]'::jsonb, true, 'en'),
('Welcome Message', 'general', 'welcome', 'YAWATU: Welcome to our platform! Your account has been activated successfully.', '[]'::jsonb, true, 'en'),
('Password Reset', 'security', 'password_reset', 'YAWATU: Your password reset code is {{reset_code}}. Valid for 15 minutes.', '["reset_code"]'::jsonb, true, 'en'),
('Transaction Alert', 'transaction', 'transaction_alert', 'YAWATU: Transaction alert - {{transaction_type}} of {{amount}} {{currency}} completed successfully.', '["transaction_type", "amount", "currency"]'::jsonb, true, 'en');

-- Create Email Templates
DELETE FROM email_templates WHERE template_type IN ('otp_verification', 'welcome', 'password_reset', 'test');

INSERT INTO email_templates (name, template_type, subject, html_content, text_content, variables, is_active) VALUES
...
'Your verification code is: {{otp_code}}. This code will expire in {{expiry_minutes}} minutes.', '["otp_code", "expiry_minutes"]'::jsonb, true),
...
'Welcome {{user_name}}! Thank you for joining our platform.', '["user_name"]'::jsonb, true),
...
'Your password reset code is: {{reset_code}}. Valid for 15 minutes.', '["reset_code"]'::jsonb, true),
...
'Test email from {{sender_name}}. Message: {{test_message}}', '["sender_name", "test_message"]'::jsonb, true);
