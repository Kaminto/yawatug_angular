-- Fix the reconciliation function to use the correct transaction types from the actual database
...
BEGIN
  ...
  LOOP
    ...
    IF calculated_holdings > 0 THEN
      ...
      INSERT INTO user_share_holdings (user_id, share_id, quantity, average_buy_price, currency)
      VALUES (
        user_record.user_id,
        (SELECT id FROM shares LIMIT 1),
        calculated_holdings,
