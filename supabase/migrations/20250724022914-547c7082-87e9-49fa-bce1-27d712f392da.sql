-- Fix SMS Provider Configuration
...
-- Insert EasyUganda provider if it doesn't exist
INSERT INTO sms_providers (
  provider_name, api_endpoint, cost_per_sms, daily_limit, monthly_limit, 
  priority, is_active, configuration
) 
SELECT 
  'EasyUganda', 
...
WHERE NOT EXISTS (SELECT 1 FROM sms_providers WHERE provider_name = 'EasyUganda');
...
INSERT INTO email_providers (name, provider_type, api_endpoint, cost_per_email, daily_limit, monthly_limit, priority, is_active) VALUES
('Resend', 'resend', 'https://api.resend.com/emails', 0.0001, 1000, 30000, 1, true)
