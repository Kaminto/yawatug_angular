-- Data Recovery Migration: Restore Lost Referral Earnings (Final Fix)
...
BEGIN
    ...
    FOR transaction_record IN
        SELECT DISTINCT
            ...
            p.referred_by as referrer_user_id
        FROM transactions t
    ...
    LOOP
        ...
        INSERT INTO referral_commissions (
            ...
            transaction_record.referrer_user_id,
    ...
    -- Update referral statistics with recovered data
    WITH commission_totals AS (
        SELECT 
            rc.referrer_id,
            COUNT(*) as total_commissions,
            SUM(rc.commission_amount) as total_earnings,
            COUNT(CASE WHEN rc.status = 'paid' THEN 1 END) as paid_commissions
...
-- Create function to validate referral data integrity
...
BEGIN
    ...
    LEFT JOIN (
        SELECT 
            rc.referrer_id,
            SUM(rc.commission_amount) as actual_total
        FROM referral_commissions rc
...
-- Create trigger to prevent future data loss during migrations
