...
-- Trading notification system admin controls
CREATE TABLE IF NOT EXISTS public.admin_trading_notification_settings (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
INSERT INTO public.admin_trading_notification_settings (email_notifications_enabled) VALUES (true) ON CONFLICT DO NOTHING;
...
ALTER TABLE public.admin_trading_notification_settings ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Admins can manage trading notification settings" ON public.admin_trading_notification_settings
  FOR ALL USING (is_admin(auth.uid())) WITH CHECK (is_admin(auth.uid()));
...
CREATE TRIGGER update_trading_notification_settings_timestamp
  BEFORE UPDATE ON public.admin_trading_notification_settings
  FOR EACH ROW EXECUTE FUNCTION update_admin_settings_timestamp();